...
WORKSHOP.site.vis.PERLIN_BLOCKS = (function PerlinBlocks(){
	var canvas,ctx,area={width:0,height:0,x:0,y:0,z:0},colors,imageData,P,fScl,running=true,pauseID,animationCount=0,mutation=0,alpha=0,swatch={x:0,y:0,z:0,width:15,height:66,depth:100},me={},animationID;
	...
	me.init = function(_c){		
		...
		canvas.onclick = me.onCanvasClick;
		...
		fScl = .02345;
		animationID = setInterval(update, 33);
		resetPauseInterval();
	};
	function resetPauseInterval(){
		clearInterval( pauseID );
		pauseID	= setInterval(me.toggleRunning, 6500);
	}
	function update(){
		if(running && animationCount < 123){
	...
	function getColors(noOfColors) {
		...
		//HEX	colors.push( '#' + WORKSHOP.utils.DRAWING.combineRGB(~~r,~~g,~~b).toString(16) );
		//ARR	colors.push([~~r,~~g,~~b]);
				colors.push( 'rgba(' + ~~r +',' + ~~g + ',' + ~~b + ',' ); //STR partial
		}				
		return colors;
	};
